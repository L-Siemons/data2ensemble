...
def relaxation_matrix_emf_c1p_no_long_range_motion(params,
		int resid, 
		spectral_density, 
		fields, 
		rxy, 
	...
	# make an emty matrix
	relaxation_matrix = np.zeros([operator_size,operator_size,  len(fields)])
	params_no_long_range_motion = params.valuesdict()
	params_no_long_range_motion['Ss'] = 1.
	params_no_long_range_motion['Sf'] = 1.
	...
	for indx, proton_i in enumerate(protons):
		for j in range(indx+1, proton_len):
			...
			dipolar_contribution = rates.r1_YX_dipollar(params_no_long_range_motion, 
				spectral_density, 
				fields,
				rxy[dist_key], 
				'h',
	...
	for indx, item in enumerate(protons):
		...
		for jndx in range(indx + 1, len(protons)):
			...
			cos_key = (resid, item , resid,  jtem)
			key = (resid, f"{item},{jtem}")
			current_noe = rates.r1_reduced_noe_YX(params_no_long_range_motion, 
						spectral_density, 
						fields,
						rxy[dist_key], 
						y, 
